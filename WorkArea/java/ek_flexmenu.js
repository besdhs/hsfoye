...
function __ekFlexMenu_static_mouseIn(e) {
	...
	if (ev) {
		...
		if (el && ("undefined" != el.id)) {
			if (0 == el.id.length) {
				el.id = __ekFlexMenu_static_getValidParentId(el);
			}
			var menuObj = ekFlexMenu.getMenuObj(el.id);
			if (menuObj) {
...
function __ekFlexMenu_showSubmenuBranch(idString) {
	...
	if (ekFlexMenu.private_isValidMenuSubmenuIdString(menuSubmenuId)) {
		...
		if (obj) {
			...
			if (null != this.lastSelectedSlaveMenuId) {
				this.unSelectSubmenu(this.lastSelectedSlaveMenuId);
			}
			...
			this.selectSubmenuHelper(menuSubmenuId);
			this.noteIfSlaveMenu(menuSubmenuId);
			result = true;
		}
	}
	return (result);
...
function __ekFlexMenu_unHideSlaveMenu() {
	var targId = null;
	if (this.lastSelectedSlaveMenuId) {
		targId = this.lastSelectedSlaveMenuId;
	}
	else {
		targId = this.ekFlexMenu_defaultMenuIdString;
	}
	if (targId) {
...
///////////////////////////////////////////////////////////
// ekFlexMenu Instance Member Helper Function.
function __ekFlexMenu_noteIfSlaveMenu(id) {
	if (id && this.ekFlexMenu_slaveStartLevelIds[id]) {
		this.lastSelectedSlaveMenuId = id;
	}
}
